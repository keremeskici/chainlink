...
var configSchema = exports_external.object({
  polymarketApiUrl: exports_external.string(),
  oracleRegistryAddress: exports_external.string(),
  chainSelectorName: exports_external.string(),
  isTestnet: exports_external.boolean(),
  owner: exports_external.string(),
  openaiModel: exports_external.string(),
  anthropicModel: exports_external.string(),
  googleModel: exports_external.string(),
  deepseekModel: exports_external.string()
});
var AI_PROVIDERS = [
  {
    name: "OpenAI",
...
function getActiveProviders(config) {
  return AI_PROVIDERS.filter((p) => config[p.modelField].trim() !== "").slice(0, 4);
}
function extractOptionsFromMarkets(markets) {
  if (!Array.isArray(markets) || markets.length === 0) {
    return ["Yes", "No"];
  }
  return markets.map((m) => m.groupItemTitle || m.title || m.question || "Unknown");
}
function mapGammaEvent(item) {
  return {
    id: item.id || "",
...
var fetchEventById = (sendRequester, config, eventId) => {
  ...
  if (resp.statusCode !== 200) {
    throw new Error(`Polymarket API returned status ${resp.statusCode} for event ${eventId}`);
  }
...
function buildSystemPrompt(event) {
  const optionsList = event.options.map((o, i2) => `  ${i2 + 1}. "${o}"`).join(`
`);
...
Available Options:
${event.options.map((o, i2) => `  ${i2 + 1}. ${o}`).join(`
`)}
...
function validateSelectedOption(selectedOption, validOptions) {
  return validOptions.includes(selectedOption);
}
function parseLLMResponse(content, validOptions) {
  ...
  if (!validateSelectedOption(parsed.selected_option, validOptions)) {
    throw new Error(`LLM returned invalid option "${parsed.selected_option}". Valid: [${validOptions.join(", ")}]`);
  }
  return {
...
function buildOpenAICompatibleRequest(provider, model, systemPrompt, userPrompt) {
...
function buildAnthropicRequest(provider, model, systemPrompt, userPrompt) {
  const body = JSON.stringify({
    model,
    max_tokens: 1000,
    system: systemPrompt,
...
function buildGoogleRequest(provider, model, systemPrompt, userPrompt) {
  const body = JSON.stringify({
...
function extractContent(providerName, responseBody) {
  switch (providerName) {
    case "Anthropic":
...
var executeSwarm = (sendRequester, config, event) => {
  ...
  for (const provider of activeProviders) {
    try {
      const model = config[provider.modelField];
      let reqConfig;
      if (provider.name === "Anthropic") {
        reqConfig = buildAnthropicRequest(provider, model, systemPrompt, userPrompt);
      } else if (provider.name === "Google") {
        reqConfig = buildGoogleRequest(provider, model, systemPrompt, userPrompt);
      } else {
        reqConfig = buildOpenAICompatibleRequest(provider, model, systemPrompt, userPrompt);
      }
      const response = sendRequester.sendRequest(reqConfig).result();
      if (!ok(response)) {
        throw new Error(`${provider.name} API returned status ${response.statusCode}`);
      }
...
function calculateConsensus(results) {
  ...
  if (successfulResults.length === 0) {
    throw new Error("Consensus failed: No successful LLM responses received.");
  }
  const voteCounts = {};
...
function executeOnChainResolution(runtime2, polymarketId, outcome) {
  ...
  if (!network248) {
    throw new Error(`Network not found for selector: ${config.chainSelectorName}`);
  }
  ...
  runtime2.log(`On-chain verdict recorded: tx=${bytesToHex(writeResult.txHash || new Uint8Array(32))}`);
}
function decodeResolutionRequestedLog(log) {
  const dataHex = `0x${Buffer.from(log.data).toString("hex")}`;
...
var onLogTrigger = (runtime2, log) => {
  ...
  runtime2.log(`ResolutionRequested received: polymarketId="${polymarketId}" tx=${txHash}`);
  runtime2.log(`Fetching Polymarket event ${polymarketId}...`);
  const httpClient = new ClientCapability3;
  const eventResult = httpClient.sendRequest(runtime2, (sendRequester, cfg) => ({
    event: fetchEventById(sendRequester, cfg, polymarketId)
  }), consensusIdenticalAggregation())(config).result();
  const event = eventResult.event;
  runtime2.log(`Event fetched: "${event.question}" (${event.options.length} options)`);
  runtime2.log("Initiating AI swarm consensus via Confidential HTTP...");
  const confHTTPClient = new ClientCapability2;
  const swarmOutput = confHTTPClient.sendRequest(runtime2, (sendRequester, cfg) => executeSwarm(sendRequester, cfg, event), consensusIdenticalAggregation())(config).result();
  for (const result of swarmOutput.results) {
    if (result.success) {
      runtime2.log(`${result.providerName}: "${result.data.selected_option}" — ${result.data.reasoning.slice(0, 100)}...`);
    } else {
      runtime2.log(`${result.providerName}: FAILED — ${result.error}`);
    }
  }
  runtime2.log("Calculating consensus...");
  const consensus = calculateConsensus(swarmOutput.results);
  if (!consensus.resolved) {
    runtime2.log(`Consensus NOT reached. Votes: ${JSON.stringify(consensus.voteCounts)}`);
    return "NO_CONSENSUS";
  }
  runtime2.log(`Consensus reached: "${consensus.winningOption}" (${consensus.totalVotes} votes: ${JSON.stringify(consensus.voteCounts)})`);
  runtime2.log(`Recording verdict on-chain: ${polymarketId} → "${consensus.winningOption}"`);
  executeOnChainResolution(runtime2, polymarketId, consensus.winningOption);
  runtime2.log("On-chain transaction confirmed.");
...
var initWorkflow = (config) => {
  ...
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const eventSignature = keccak256(toBytes("ResolutionRequested(string,address)"));
  const logTrigger = evmClient.logTrigger({
    addresses: [hexToBase64(config.oracleRegistryAddress)],
    topics: [
